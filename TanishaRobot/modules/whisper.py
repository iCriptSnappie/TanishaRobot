...
from TanishaRobot import BOT_USERNAME
...
async def _whisper(_, inline_query):
    ...
    if len(data.split()) < 2:
        ...
        try:
            ...
            one_time_whisper_btn = InlineKeyboardMarkup([[InlineKeyboardButton("💌 sʜᴏᴡ ᴍᴇssᴀɢᴇ", callback_data=f"fdaywhisper_{inline_query.from_user.id}_{user.id}_one")]])
            mm = [
                InlineQueryResultArticle(
                    title="💌 ᴡʜɪsᴘᴇʀ",
...
async def in_help():
    answers = [
        InlineQueryResultArticle(
            title="💌 ᴡʜɪsᴘᴇʀ",
            description=f"@TanishaRobot [USERNAME | ID] [TEXT]",
            input_message_content=InputTextMessageContent(f"**⬤ ᴜsᴀɢᴇ ➥**\n\n@TanishaRobot (ᴛᴀʀɢᴇᴛ ᴜsᴇʀɴᴀᴍᴇ ᴏʀ ɪᴅ) (ʏᴏᴜʀ ᴍᴇssᴀɢᴇ).\n\n**๏ ᴇxᴀᴍᴘʟᴇ ➠**\n@TanishaRobot @username ɪ ᴡᴀɴɴᴀ ғᴜᴄᴋ ʏᴏᴜ"),
            thumb_url="https://telegra.ph/file/21c69049c4855ac0a035b.jpg",
            reply_markup=switch_btn
        )
    ]
...
⬤ @TanishaRobot [@username] || [id] ʏᴏᴜʀ ᴍᴇssᴀɢᴇ
"""
